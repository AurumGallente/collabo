					...
					foreach($usedImages as $file => $val){
						if($useDummy == "true"){ //only use dummy images
							...
							if(strpos($file, 'http') !== false){
								...
								$blogEnd = strpos($file, $blogUrl);
								$tFile = substr($file, $blogEnd);
								$zip->addFile($file, 'images/'.$tFile);*/
		...
		public function importSliderFromPost($updateAnim = true, $updateStatic = true){
			...
			try{
				...
				foreach($arrSlides as $slide){
					...
					$arrCreate["layers"] = stripslashes(json_encode($layers));
					$arrCreate["params"] = stripslashes(json_encode($params));
